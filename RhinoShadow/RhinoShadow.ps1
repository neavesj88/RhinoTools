...
try {
    ...
    # ------------------------------------------------------------------
    # Click the rhino. The first 10 clicks pull from $friendlyMessages
    # (cheery IT-isms and shoutouts). Clicks 11-20 pull from
    # $irritatedMessages (Rhino is getting tired of you). Clicks 21+
    # pull from $blanketStatements (Rhino has fully checked out).
    # $script:lastMessage prevents the same line appearing twice in a
    # row (the no-repeat guard).
    #
    # Message arrays are a verbatim copy from RhinoCopy so the easter
    # egg feels identical across the toolset. The shared library lives
    # at Shared/SecretRhinoMessages.ps1 if you want to drop this into
    # another tool.
    $script:clickCount = 0
    $script:lastMessage = ""
    $friendlyMessages = @(
    ...
    # Click handler. Bumps the counter, picks a non-repeating message
    # from whichever mood array is active for the current click count,
    # and writes it to the activity log so long lines have room to
    # display. The do...while + lastMessage check prevents the same
    # message appearing twice in a row. Write-RhinoLog isn't defined
    # until SECTION 6i, but the closure resolves it at click time so
    ...
    $mascot.Add_Click({
        ...
        Write-RhinoLog "Secret Rhino Message: $newMessage"
    })
