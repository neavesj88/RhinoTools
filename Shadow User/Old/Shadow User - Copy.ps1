...
function Show-Users {
    ...
    if ($result -eq [Windows.Forms.DialogResult]::OK) {
        ...
        if ($selectedRowIndex -ge 0) {
            ...
            $selectedUser = $usersList | Where-Object { $_.SessionID -eq $selectedSessionID }
            if ($selectedUser) {
                Write-Host "Selected User: $($selectedUser.Username), Session ID: $($selectedUser.SessionID), RDS Server: $($selectedUser.Server)"
                $response = [Windows.Forms.MessageBox]::Show("Do you want to shadow $($selectedUser.Username)?", "Shadow User", [Windows.Forms.MessageBoxButtons]::YesNo, [Windows.Forms.MessageBoxIcon]::Question)
                if ($response -eq [Windows.Forms.DialogResult]::Yes) {
                    mstsc /v:$($selectedUser.Server) /shadow:$($selectedUser.SessionID) /control
                }
            } else {
