...
function Get-LoggedUsersFromServer {
   ...
   $users = @()
   try {
       # Run query user and capture the session information
       $sessions = query user /server:$serverName
       
       # Filter out header or non-session lines
       $sessions = $sessions | Where-Object { $_ -notmatch '^ USERNAME' }

       foreach ($session in $sessions) {
           # Split the session info by whitespace to extract user properties
           $userProperties = $session -split '\s+', 7 
           
           if ($userProperties.Length -ge 7) {
               $objectProperties = [ordered]@{
                   Username = $userProperties[1]
                   SessionID = $userProperties[3]
                   LogonTime = $userProperties[6]
                   Server = $serverName
               }
...
function Show-MainForm {
   ...
   function Show-Users {
       ...
       $usersGridView.Columns["Username"].Width = 160
       $usersGridView.Columns["LogonTime"].Width = 120
       $usersGridView.Columns["Server"].Width = 130
       # Add each user session to the grid view (sorted alphabetically)
       $usersList | Sort-Object Username | ForEach-Object {
           $usersGridView.Rows.Add($_.Username, $_.Server, $_.LogonTime, $_.SessionID)
       }
       # Add a button to refresh the user list
       $refreshButton = New-Object Windows.Forms.Button
       $refreshButton.Location = New-Object Drawing.Point(250, 340)
       $refreshButton.Size = New-Object Drawing.Size(110, 30)
       ...
       $refreshButton.Add_Click({
        ...
        # Add a button to sign out the selected user
       $signOutButton = New-Object Windows.Forms.Button
       $signOutButton.Location = New-Object Drawing.Point(130, 340)
       $signOutButton.Size = New-Object Drawing.Size(110, 30)
       ...
       $signOutButton.Add_Click({
           ...
           if ($selectedRowIndex -ge 0) {
               ...
               Write-Host "Signing out user: $($usersGridView.Rows[$selectedRowIndex].Cells["Username"].Value) from server: $selectedServer"
               $response = [Windows.Forms.MessageBox]::Show("Do you want to sign out $($usersGridView.Rows[$selectedRowIndex].Cells["Username"].Value)?", "Shadow User", [Windows.Forms.MessageBoxButtons]::YesNo, [Windows.Forms.MessageBoxIcon]::Question)
               if ($response -eq [Windows.Forms.DialogResult]::Yes) {
               logoff $selectedSessionID /server:$selectedServer
               }
           } else {
               [System.Windows.Forms.MessageBox]::Show("Please select a user first.", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
           }
       })
       # Add a button to send a message to all users on the selected servers
       $sendMessageButton = New-Object Windows.Forms.Button
       $sendMessageButton.Location = New-Object Drawing.Point(10, 380)
       $sendMessageButton.Size = New-Object Drawing.Size(470, 30)
       $sendMessageButton.Text = "Send Message to All Users"
       $sendMessageButton.Add_Click({
           $message = [Windows.Forms.MessageBox]::Show("Enter the message to send to all users:", "Message to Users", [Windows.Forms.MessageBoxButtons]::OKCancel)
           if ($message -ne [Windows.Forms.DialogResult]::Cancel) {
               foreach ($server in $selectedServers) {
                   msg * "$message" /server:$server
               }
           }
       })
       # Add a "Back" button to return to the main form
       $backButton = New-Object Windows.Forms.Button
       $backButton.Location = New-Object Drawing.Point(10, 340)
       $backButton.Size = New-Object Drawing.Size(110, 30)
       ...
       $okButton.Location = New-Object Drawing.Point(370, 340)
       $okButton.Size = New-Object Drawing.Size(110, 30)
       ...
       $usersForm.Controls.Add($signOutButton)
       $usersForm.Controls.Add($backButton)
       #$usersForm.Controls.Add($sendMessageButton)
   ...
   $dropDown.add_SelectedIndexChanged({
       ...
       $servers = Get-ServersFromOU $ouPath | Where-Object {$_ -notlike "*sh77*"}
       $listBox.Items.Clear()
...
# Show the main form on script start
Show-MainForm